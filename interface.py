...
with gr.Blocks() as demo:
    ...
    subtasks = gr.Textbox(label="Decomposed Sub-Questions", lines=2)

    compiled_ans = gr.Textbox(label="Compiled Answer", lines=2)

    btn = gr.Button("Decompose Question")
    btn.click(fn=pipe.run, inputs=user_input, outputs=[subtasks, compiled_ans])
